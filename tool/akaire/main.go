...
func main() {
	...
	mux.HandleFunc("POST /api/review", func(w http.ResponseWriter, r *http.Request) {
		...
		prompt += fmt.Sprintf(viewpointsSuffixFmt, viewpointsPath, string(viewpoints))
		review.Lock()
		defer review.Unlock()
		if review.running {
...
// viewpointsPath は赤入れの観点をまとめたファイル (-data 相対)。
// 「何を見るか・何を指摘しないか・どう書くか」はすべてここにあり、
// GitHub Actions 上の @claude (同ディレクトリの SKILL.md) と共有している。
...
// ここで埋め込むのは、読み忘れを起こさないため。
const viewpointsSuffixFmt = `

----
以下は赤入れの観点 (%[1]s の内容)。上の手順で「観点は viewpoints の n 章」と
指示している箇所はこれを指す。この観点に従って指摘すること。

%[2]s`

// annotationFormat は .akaire.json への指摘 1 件の形式 (各プロンプトで共通)
...
3. 変更箇所を読み、指摘を %[2]s の annotations 配列に追記する
   ...
   観点は viewpoints の 1 章 (文の成立)、2 章 (文体)、3 章 (構成・読みやすさ)、5 章 (事実確認)。
...
3. 全文を通して読み、指摘を %[2]s の annotations 配列に追記する
   (ファイルが無ければ {"annotations": []} から作る)。
   観点は viewpoints の 1 章 (文の成立)、2 章 (文体)、3 章 (構成・読みやすさ)、
   4 章 (通しでしか見つからない観点)、5 章 (事実確認・公開前チェック)。4 章と 5 章を特に重視する。
...
3. 指摘を %[2]s の annotations 配列に追記する
   ...
   - 記事全体に関わる指摘 → 先頭の見出し (# …) の行を quote にする
     (見出しが無い原稿ではフロントマターを除く本文の先頭の一文)
   - 章に関わる指摘 → その章の見出し (## …) の行を quote にする
   - 特定の段落に関わる指摘 → その段落の冒頭の一文を quote にする
     (構成の立ち上げで章ごとに 1 件にまとめる場合は、その章に入る最初の段落の冒頭の一文)
   kind はすべて "pencil"。代案は方向性の提示に留め、実際の文章は筆者が書く。
...
2. %[4]s の annotations 配列に、この相談への返しを 1 件だけ追記する
   ...
   - body の書き方は viewpoints の 7 章 (相談専用) に従う: 診断 → 方向性の違う案 2〜3 個 →
     今のままでも成立するならそう言う
